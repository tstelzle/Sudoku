...
class RecursiveBacktracking(ProblemFinder):
    ...
    def recursive(self, x: int, y: int, valIndex: int, tried: int, count: int):
        if (y == self.board.getBoardLength() and x == 0):
            return

        if (y < self.board.getBoardLength() and x < self.board.getBoardLength()):
            val = self.setValues[self.getIdentifier(x, y)][valIndex]
            ...
            if (row and column and box):
                ...
                if x == self.board.getBoardLength() - 1:
                    return self.recursive(0, y+1, 0, 1, count+1)
                else:
                    return self.recursive(x+1, y, 0, 1, count+1)
            elif tried >= len(self.setValues[self.getIdentifier(x, y)]):
                self.board.setValue(x, y, -1)
                if x == 0:
                    return self.recursive(self.board.getBoardLength()-1, y-1, 0, 1, count+1)
                else:
                    return self.recursive(x-1, y, 0, 1, count+1)
            else:
                return self.recursive(x, y, valIndex+1, tried+1, count+1)
